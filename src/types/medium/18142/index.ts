...
type All<T extends unknown[], P> = T extends [infer L, ...infer R]
  ? Equal<P, L> extends true
    ? All<R, P>
    : false
  : true;
...
/*
 この型定義は、与えられたジェネリック型Tのすべての要素が型Pと等しいかどうかをチェックする型
 1. inferを使って配列を分割
 T extends [infer L, ...infer R] で配列を分割し、Equalを使って最初の要素とPが等しいか比較する。
 PとLが等しければ残りの要素に対してAllを再帰的に適用し、そうでなければfalseを返す。
 */
