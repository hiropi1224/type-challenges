...
/*
 この型定義は、与えられたタプル内に重複した要素が存在するかどうかをチェックするための型
 1. inferを使ってTを分割
 T extends [infer L, ...infer R] ?として、LがR[number]と互換性があるかをチェックし、互換性があればtrueを返す。
 そうでなければ残りの要素に対してCheckRepeatedTupleを再帰的に適用する。
 */
