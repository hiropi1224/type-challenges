...
type PercentageParser<A extends string> = A extends `${infer T extends
  | '+'
  | '-'}${infer R}%`
  ? [T, R, '%']
  : A extends `${infer T extends '+' | '-'}${infer R}`
  ? [T, R, '']
  : A extends `${infer R}%`
  ? ['', R, '%']
  : A extends `%`
...
/*
 1. Aが`${infer T extends '+'|'-'}${infer R}%`にマッチする場合、[T, R, '%']を返す
 2. Aが`${infer T extends '+' | '-'}${infer R}`にマッチする場合、[T, R, '']を返す
 3. Aが`${infer R}%`にマッチする場合、['', R, '%']を返す
 4. Aが`%`にマッチする場合、['', '', '%']を返す
 5. Aが`${infer R}`にマッチする場合、['', R, '']を返す
 6. 上記にマッチしない場合、['','','']を返す
*/
