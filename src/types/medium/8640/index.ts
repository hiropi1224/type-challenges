...
/*
 この型定義は、指定された範囲内の整数のセットを表現する型を生成するための型
 1. 長さがNである配列を生成する
 T['length'] extends Nとして、Tの長さがNに一致するかをチェックし、一致すればTの長さを返す。
 そうでなければTに現在の配列の長さを追加して再帰的に呼び出す。
 2. 範囲の開始値と終了値の長さの配列を生成
 TupleOfLengthを使って範囲の開始値と終了値の長さを持つ配列を生成し、Exclude<TupleOfLength<H>, TupleOfLength<L>>とする。
 こうすることでHにあってLにない整数のセットを生成する。
 */
